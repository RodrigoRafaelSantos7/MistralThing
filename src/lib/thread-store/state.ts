...
import type { ThreadStore } from "@/lib/thread-store/store";

export class ThreadState<UI_MESSAGE extends UIMessage>
  implements ChatState<UI_MESSAGE>
{
  private readonly store: ThreadStore<UI_MESSAGE>;
  ...
  snapshot = <T>(snapshot: T) => structuredClone(snapshot);
}
