...
export function ClassicLoader({
  ...
  const CLASSIC_LOADER_BARS_COUNT = 12;
  const ROTATION_DEGREES_PER_BAR = 30;
  ...
  return (
    <div className={cn("relative", sizeClasses[size], className)}>
      <div className="absolute h-full w-full">
        {[...new Array(CLASSIC_LOADER_BARS_COUNT)].map((_, i) => (
          <div
            className="absolute animate-[spinner-fade_1.2s_linear_infinite] rounded-full bg-primary"
            // biome-ignore lint/suspicious/noArrayIndexKey: Static array for visual elements
            key={`classic-bar-${i}`}
            style={{
              top: "0",
              left: "50%",
              marginLeft: getMarginLeft(),
              transformOrigin: `${getTransformOriginX()} ${getTransformOriginY()}`,
              transform: `rotate(${i * ROTATION_DEGREES_PER_BAR}deg)`,
              opacity: 0,
...
export function DotsLoader({
  ...
  const DOTS_COUNT = 3;
  const DOTS_ANIMATION_DELAY_MS = 160;
    ...
    >
      {[...new Array(DOTS_COUNT)].map((_, i) => (
        <div
          ...
          key={`dots-${i}`}
          style={{
            animationDelay: `${i * DOTS_ANIMATION_DELAY_MS}ms`,
          }}
        />
      ))}
      <span className="sr-only">Loading</span>
...
export function TypingLoader({
  ...
  const TYPING_ANIMATION_DELAY_MS = 250;
  ...
  return (
    ...
    >
      {[...new Array(TYPING_DOTS_COUNT)].map((_, i) => (
        <div
          className={cn(
          ...
          key={`typing-${i}`}
          style={{
            animationDelay: `${i * TYPING_ANIMATION_DELAY_MS}ms`,
          }}
        />
      ))}
      <span className="sr-only">Loading</span>
...
export function WaveLoader({
  ...
  const WAVE_BARS_COUNT = 5;
  const WAVE_ANIMATION_DELAY_MS = 100;
  ...
  return (
    ...
    >
      {[...new Array(WAVE_BARS_COUNT)].map((_, i) => (
        <div
          className={cn(
          ...
          key={`wave-${i}`}
          style={{
            animationDelay: `${i * WAVE_ANIMATION_DELAY_MS}ms`,
...
export function BarsLoader({
  ...
  const BARS_COUNT = 3;
  const BARS_ANIMATION_DELAY_SECONDS = 0.2;

  return (
    <div className={cn("flex", containerSizes[size], className)}>
      {[...new Array(BARS_COUNT)].map((_, i) => (
        <div
          className={cn(
          ...
          key={`bars-${i}`}
          style={{
            animationDelay: `${i * BARS_ANIMATION_DELAY_SECONDS}s`,
          }}
        />
      ))}
      <span className="sr-only">Loading</span>
